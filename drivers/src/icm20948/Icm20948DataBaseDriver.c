	...
	const uint8_t *dmp3_image, uint32_t dmp3_image_size)
{
    ...
    data = 0xD0;
    result |= inv_icm20948_write_mems_reg(s, REG_INT_PIN_CFG, 1, &data);
	...
	s->base_state.accel_averaging = 2;                      //Change this value if higher sensor sample avergaing is required.
	s->base_state.gyro_averaging = 4;                       //Change this value if higher sensor sample avergaing is required.
...
int inv_icm20948_set_icm20948_gyro_fullscale(struct inv_icm20948 * s, int level)
{
	...
	gyro_config_1_reg |= (level << 1) | 1;  //fchoice = 1,
    gyro_config_1_reg |= (2 << 3) | 1;  //filter = 2
...
int inv_icm20948_set_icm20948_accel_fullscale(struct inv_icm20948 * s, int level)
{
	...
	if(s->base_state.accel_averaging > 1)
		accel_config_1_reg |= (2 << 3) | (level << 1) | 1;   //fchoice = 1, filter = 2.
	else
